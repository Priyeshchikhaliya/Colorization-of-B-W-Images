...
class Window(Frame):
    ...
    def uploadImage(self):
        ...
        load = load.resize((360, 360), Image.ANTIALIAS)
        ...
        cv.imwrite("./Colorized.png", (bgr_output*255).astype(np.uint8))
    def color(self):
        load = Image.open("./Colorized.png")
        load = load.resize((360, 360), Image.ANTIALIAS)
        if self.image is None:
            w, h = load.size
            self.render = ImageTk.PhotoImage(load)
...
root = tk.Tk()
root.geometry("%dx%d" % (1200, 600))
root.title("B&W Image Colorization GUI")
img = ImageTk.PhotoImage(Image.open("./Extra/albert_einstein.jpg"))
app = Window(root)
